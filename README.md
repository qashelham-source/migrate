...
For duplicates created before the detector was enabled, open **Smart Center → Duplicate Detector → Scan Destination**. The manager's existing user session reads the configured destination history, groups messages with the exact same Telegram `file_unique_id` in the same destination and topic, and shows a read-only preview. If matching media was re-uploaded with a different Telegram fingerprint, use **Deep Scan Same Files**: it downloads only same-type, same-size candidates and compares their SHA-256 file checksums. It never deletes merely similar or re-encoded media. A separate delete confirmation queues deletion through that same manager session; the control bot only shows the controls. Only later matching destination messages are removed, while the oldest copy and every source post stay untouched. Stop an active migration before scanning or cleanup, keep the manager service running, and ensure the manager user session can delete messages in that destination.
